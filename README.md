...
- **Swarm**: every session gets a `color-animal` address. Sessions can
  ...
  needed — optionally on a different model. Workers can be pinned to a
  sibling worktree; recorded lineage nests them under their orchestrator
  in the sidebar.
...
- Updating the binary preserves in-flight work: the old server drains and
  hands its queue off to the new one (see [Updating the
